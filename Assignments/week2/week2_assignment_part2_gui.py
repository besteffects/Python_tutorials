...
for f in EXPECTED_FNS:
    if not hasattr(week2_assignment_part1, f):
        print(f, "is missing in week2_assignment_part1.py.")
        week2_assignment_part1.__dict__[f] = lambda *x: 0
...
class Clock(object):
    ...
    def __init__(self, tk, zone=None, **kargs):
        ...
        self.diff = week2_assignment_part1.hours_difference(dlt, 0)
    ...
    def refresh(self, forced=False):
        ...
        hour = int(week2_assignment_part1.time_from_utc(utc_hour, self.diff))
        ...
        minutes_angle = week2_assignment_part1.get_minutes(int(self.now)) / 60 * 360
        if week2_assignment_part1.time_from_utc(0, self.diff) % 1 == 0.5:
            minutes_angle += 180
        elif week2_assignment_part1.time_from_utc(0, self.diff) % 1 == 0.75:
            minutes_angle += 270
        ...
        seconds_angle = int(week2_assignment_part1.get_seconds(int(self.now))) / 60 * 360
